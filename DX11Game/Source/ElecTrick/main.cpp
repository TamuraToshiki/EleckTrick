...
#define WINDOW_NAME		_T("ElecTrick")			// ウインドウのキャプション名
...
DXGI_SAMPLE_DESC g_MSAA;							// MSAA マルチサンプリング
...
void Draw(void)
{
	...
	// ポストエフェクトバッファ
	g_pDeviceContext->ClearRenderTargetView(g_postEffect.m_pRenderTargetView, ClearColor);
	g_pDeviceContext->ClearDepthStencilView(g_postEffect.m_pDepthStencilView,
		D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);


	// 各ターゲットビューをレンダーターゲットに設定
	//g_pDeviceContext->OMSetRenderTargets(1, &g_pRenderTargetView, g_pDepthStencilView);
	g_pDeviceContext->OMSetRenderTargets(1, &g_postEffect.m_pRenderTargetView,
		g_postEffect.m_pDepthStencilView);
	...
	// モデルの描画
	CRenderer::ListDraw(g_pDeviceContext);
