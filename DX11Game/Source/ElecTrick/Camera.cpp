...
#define	CAM_POS_P_Z			(-500.0f * 1.3f)				// カメラの視点初期位置(Z座標)
...
void CCamera::Init()
{
	...
	m_fLengthOffset = 0.0f;

	m_vOffSet = XMFLOAT3{ 0,0,0 };
...
void CCamera::Update()
{
	...
	// 距離
	m_fLengthInterval = m_fInitLengthInterval + m_fLengthOffset;

	// カメラ移動
	//if (GetKeyPress(VK_Q)) {
	//	m_fLengthOffset -= 5.0f;
	//	if (m_fLengthOffset < CAM_DIST_MIN + m_fInitLengthInterval)
	//		m_fLengthOffset = CAM_DIST_MIN + m_fInitLengthInterval;
	//}
	//if (GetKeyPress(VK_E)) {
	//	m_fLengthOffset += 5.0f;
	//	if (m_fLengthOffset > CAM_DIST_MAX)
	//		m_fLengthOffset = CAM_DIST_MAX;
	//}
	XMFLOAT3 vDir;
	XMStoreFloat3(&vDir, XMVector3Normalize(
		XMVectorSet(0.0f,
			0.0f, m_vPos.z - m_vTarget.z,
			0.0f)));
